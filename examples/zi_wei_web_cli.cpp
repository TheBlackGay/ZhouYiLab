    ...
    json rule_profile() {
        return {
            ...
            {"rules", {
                ...
                {"brightness",
                    "亮度分层赋格：引擎原生仅十四主星按星曜×宫位地支固定取表，七等庙旺得平陷不利，"
                    "CLI 原始输出中辅星、煞星、杂曜亮度为空；HTTP 适配层再按版本化星曜亮度表"
                    "（brightness_table_version）回填空星、煞星与杂曜中的有效星，"
                    "台辅、天厨、天巫、天月、封诰、月德、空亡、蜚廉、阴煞、龙德等神煞类杂曜"
                    "不赋亮度（实现以代码为准）"},
                {"fortune_layers",
