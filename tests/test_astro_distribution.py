...
class DistributionRouteContractTests(unittest.TestCase):
    ...
    def test_server_and_frontend_wired(self):
        ...
        shared = (ROOT / "web" / "profile-charts.js").read_text(encoding="utf-8")
        self.assertIn("/api/v1/astro/distribution", javascript)
        self.assertIn("window.ProfileCharts", javascript)
        self.assertIn("render(container, distribution)", shared)
        self.assertIn("conic-gradient", shared)
